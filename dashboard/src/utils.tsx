...
export type OrError = { status: "ok" } | { status: "error", err: any } | { status: "working" };
...
export function OrError(props: { get: Accessor<OrError> }) {
    return <>
        <Show when={props.get().status == "error"}>
            <div class="text-red-500 text-xs px-1">{(props.get() as { err:any }).err}</div>
        </Show>
        <Show when={props.get().status == "working"}>
            <div class="text-slate-600 text-xs px-1">Working...</div>
        </Show>
    </>
}
